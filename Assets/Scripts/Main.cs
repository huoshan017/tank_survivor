...
public class Main : MonoBehaviour
{
  ...
  void UpdateCameraPos()
  {
    ...
    if (gameObj != null)
    {
      ...
      if (lastUpdateFrameNum_ == 0 || currFrame - lastUpdateFrameNum_ > updateCameraFrameNum)
      {
        ...
        if (lastUpdateFrameNum_ == 0)
        {
          // 设置相机位置
          Camera.main.transform.position = new Vector3(x, y, originalZ);
          currCameraPosition_ = Camera.main.transform.position;
          lastCameraPosition_ = currCameraPosition_;
        }
        else
        {
          lastCameraPosition_ = currCameraPosition_;
          currCameraPosition_ = new Vector3(x, y, originalZ);
        }
        lastUpdateFrameNum_ = currFrame;
      }
      else
      {
          Camera.main.transform.position = Vector3.Lerp(lastCameraPosition_, currCameraPosition_, (currFrame-lastUpdateFrameNum_)/updateCameraFrameNum);
      }*/
    }
  }
  ...
  //Vector3 lastCameraPosition_, currCameraPosition_;
  //int lastUpdateFrameNum_;
  //const uint updateCameraFrameNum = 10;
}
