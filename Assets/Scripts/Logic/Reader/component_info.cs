...
namespace Logic.Reader
{
  public class ComponentInfoReader
  {
    ...
    /*public static void ParseAABB(YamlMappingNode aabbNode, AABBCompDef compDef, AABBComponent comp)
    {
      foreach (var n in aabbNode)
      {
        string key = ((YamlScalarNode)n.Key).Value;
        if (key == "width")
        {
          if (compDef != null)
          {
            compDef.Width = int.Parse(n.Value.ToString());
          }
          if (comp != null)
          {
            comp.Width = int.Parse(n.Value.ToString());
          }
        }
        else if (key == "height")
        {
          if (compDef != null)
          {
            compDef.Height = int.Parse(n.Value.ToString());
          }
          if (comp != null)
          {
            comp.Height = int.Parse(n.Value.ToString());
          }
        }
        else if (key == "is_rigidbody")
        {
          if (compDef != null)
          {
            compDef.IsRigidBody = bool.Parse(n.Value.ToString());
          }
          if (comp != null)
          {
            comp.IsRigidBody = bool.Parse(n.Value.ToString());
          }
        }
      }
    }*/
